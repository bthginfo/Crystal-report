...
// event requires it)
type TsEventInit<T> = T extends keyof GlobalEventHandlersEventMap
    ? GlobalEventHandlersEventMap[T] extends CustomEvent<Record<PropertyKey, unknown>>
        ? GlobalEventHandlersEventMap[T] extends CustomEvent<Record<PropertyKey, never>>
...
export default class ComponentElement extends LitElement {
    ...
    @property() override dir: string = 'ltr';
    @property() override lang: string = 'en';

    /** Emits a custom event with more convenient defaults. */
    emit<T extends string & keyof EventTypesWithoutRequiredDetail>(
        name: EventTypeDoesNotRequireDetail<T>,
        options?: TsEventInit<T> | undefined,
    ): GetCustomEventType<T>;
    emit<T extends string & keyof EventTypesWithRequiredDetail>(
        name: EventTypeRequiresDetail<T>,
        options: TsEventInit<T>,
    ): GetCustomEventType<T>;
    ...
    /* eslint-disable */
    static version = '__PACKAGE_VERSION__' in globalThis ? (globalThis as any).__PACKAGE_VERSION__ : '(dev)';
    ...
    static define(name: string, elementConstructor = this, options: ElementDefinitionOptions = {}) {
        ...
        if (!currentlyRegisteredConstructor) {
            ...
            // we fall back to anonymous classes.
            // customElements can only have 1 class of the same "object id" per registry,
            // so that is why the try {} catch {} exists.
            // Some tools like Jest Snapshots and if you import the constructor and
            // call `new TsButton()` they will fail with the anonymous class version.
            try {
        ...
        console.warn(
            `Attempted to register <${name}>${newVersion}, 
                     but <${name}>${existingVersion} has already been registered.`,
        );
    }
    ...
    constructor() {
        ...
        const ctor = this.constructor as typeof ComponentElement;
    ...
    override attributeChangedCallback(name: string, oldValue: string | null, newValue: string | null) {
        if (!this.#hasRecordedInitialProperties) {
            (this.constructor as typeof ComponentElement).elementProperties.forEach((obj, key) => {
                const prop = key as keyof this;

                if (obj.reflect && this[prop] != null) {
                    this.initialReflectedProperties.set(prop as string, this[prop]);
                }
            });
    ...
    protected override willUpdate(changedProperties: Parameters<LitElement['willUpdate']>[0]): void {
        super.willUpdate(changedProperties);

        // Run the morph fixing *after* willUpdate.
        this.initialReflectedProperties.forEach((value, key) => {
...
export interface ComponentFormControl extends ComponentElement {
    // Form attributes
    name: string;
    value: unknown;
    disabled?: boolean;
