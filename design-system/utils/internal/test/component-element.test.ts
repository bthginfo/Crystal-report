...
// We don't use ComponentElement directly because it shouldn't exist in the final bundle.
...
const ComponentElement = Object.getPrototypeOf(TsButton);
...
it('Should provide a console warning if attempting to register the same tag twice', () => {
    class MyButton extends TsButton {
        static override version = '0.4.5';
    }

    const stub = Sinon.stub(console, 'warn');

    expect(Boolean(window.customElements.get('ts-button'))).to.be.false;

    TsButton.define('ts-button');
    expect(Boolean(window.customElements.get('ts-button'))).to.be.true;
    MyButton.define('ts-button');
    ...
    expect(warning).to.match(
        new RegExp(
            `Attempted to register <ts-button> v${MyButton.version}, but <ts-button> v${TsButton.version} has already been registered`,
        ),
        'i',
    );
});
...
it('Should register dependencies when the element is constructed the first time', () => {
    class MyElement extends ComponentElement {
        static dependencies = { 'ts-button': TsButton };
    ...
    new (window.customElements.get('ts-element'))();
