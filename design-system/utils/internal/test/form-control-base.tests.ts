...
type CreateControlFn = () => Promise<ComponentFormControl>;

/** Runs a set of generic tests for Shoelace form controls */
export function runFormControlBaseTests<T extends ComponentFormControl = ComponentFormControl>(
    tagNameOrConfig:
        | string
        | {
              tagName: string;
...
function runAllValidityTests(
    tagName: string, //
    displayName: string,
    createControl: () => Promise<ComponentFormControl>,
) {
    ...
    describe(`Form validity base test for ${displayName}`, async () => {
        ...
        it('should not emit an `ts-invalid` event when `.checkValidity()` is called while valid', async () => {
            const control = await createControl();
            const emittedEvents = checkEventEmissions(control, 'ts-invalid', () => control.checkValidity());
            expect(emittedEvents.length).to.equal(0);
        });

        it('should not emit an `ts-invalid` event when `.reportValidity()` is called while valid', async () => {
            const control = await createControl();
            const emittedEvents = checkEventEmissions(control, 'ts-invalid', () => control.reportValidity());
            expect(emittedEvents.length).to.equal(0);
        });

        // TODO: As soon as `SlRadioGroup` has a property `disabled` this
        // condition can be removed
        if (tagName !== 'ts-radio-group') {
            it('should not emit an `ts-invalid` event when `.checkValidity()` is called in custom error case while disabled', async () => {
                const control = await createControl();
            ...
            it('should not emit an `ts-invalid` event when `.reportValidity()` is called in custom error case while disabled', async () => {
                const control = await createControl();
                control.setCustomValidity('error');
                control.disabled = true;
                await control.updateComplete;
        ...
        if (mode === 'tsButtonOfTypeButton') {
            runSpecialTests_slButtonOfTypeButton(createControl);
        } else if (mode === 'tsButtonWithHRef') {
...
//
//  Special tests for <ts-button type="button">
//
function runSpecialTests_slButtonOfTypeButton(createControl: CreateControlFn) {
    ...
    it('should not emit an `ts-invalid` event when `.checkValidity()` is called in custom error case, and not disabled', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
    ...
    it('should not emit an `ts-invalid` event when `.reportValidity()` is called in custom error case, and not disabled', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
        control.disabled = false;
        await control.updateComplete;
...
//
// Special tests for <ts-button href="...">
//
function runSpecialTests_slButtonWithHref(createControl: CreateControlFn) {
    ...
    it('should not emit an `ts-invalid` event when `.checkValidity()` is called in custom error case', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
        await control.updateComplete;
        const emittedEvents = checkEventEmissions(control, 'ts-invalid', () => control.checkValidity());
    ...
    it('should not emit an `ts-invalid` event when `.reportValidity()` is called in custom error case', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
        await control.updateComplete;
        const emittedEvents = checkEventEmissions(control, 'ts-invalid', () => control.reportValidity());
...
async function createFormControl<T extends ComponentFormControl = ComponentFormControl>(tagName: string): Promise<T> {
...
function checkEventEmissions(control: ComponentFormControl, eventType: string, action: () => void): Event[] {
...
// Component `ts-button` behaves quite different to the other components. To keep things simple we use simple conditions
// here. `ts-button` might stay the only component in Shoelace core behaves that way, so we just hard code it here.
function getMode(control: ComponentFormControl) {
    if (
        control.localName === 'ts-button' && //
        'href' in control &&
        'type' in control &&
        control.type === 'button' &&
        !control.href
    ) {
        return 'tsButtonOfTypeButton';
    }

    // <ts-button href="...">
    if (control.localName === 'ts-button' && 'href' in control && !!control.href) {
        return 'tsButtonWithHRef';
    }
