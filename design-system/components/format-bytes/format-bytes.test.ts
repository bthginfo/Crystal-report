...
import type { TsFormatBytes } from './index.js';
import '@tuvsud/design-system/format-bytes';

describe('<ts-format-bytes>', () => {
    describe('defaults ', () => {
        it('default properties', async () => {
            const el = await fixture<TsFormatBytes>(html`<ts-format-bytes></ts-format-bytes>`);
    ...
    describe('bits', () => {
        ...
        results.forEach(expected => {
            it('bits : display formats', async () => {
                const el = await fixture<TsFormatBytes>(html` <ts-format-bytes unit="bit"></ts-format-bytes> `);
                // short
                el.value = expected.value;
                await elementUpdated(el);
                expect(el.shadowRoot?.textContent).to.equal(expected.short);
