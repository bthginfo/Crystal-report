...
import { type HasSlotController } from '../../../utils/internal/slot.js';
import { html } from 'lit';
import type { ReactiveController, ReactiveControllerHost } from 'lit';
import type { TsMenuItem } from '././TsMenuItem.js';
import type { TsPopup } from '../../popup/index.js';
...
export class SubmenuController implements ReactiveController {
    private host: ReactiveControllerHost & TsMenuItem;
    private popupRef: Ref<TsPopup> = createRef();
    ...
    constructor(host: ReactiveControllerHost & TsMenuItem, hasSlotController: HasSlotController) {
    ...
    private addListeners() {
        ...
        if (!this.isPopupConnected) {
            if (this.popupRef.value) {
                ...
                this.popupRef.value.addEventListener('ts-reposition', this.handlePopupReposition);
    ...
    private removeListeners() {
        ...
        if (this.isPopupConnected) {
            if (this.popupRef.value) {
                ...
                this.popupRef.value.removeEventListener('ts-reposition', this.handlePopupReposition);
    ...
    private handleSubmenuEntry(event: KeyboardEvent) {
        ...
        for (const elt of submenuSlot.assignedElements()) {
            menuItems = elt.querySelectorAll("ts-menu-item, [role^='menuitem']");
    ...
    private handleClick = (event: MouseEvent) => {
        ...
        if (event.target === this.host) {
            ...
            (event.target.tagName === 'ts-menu-item' || event.target.role?.startsWith('menuitem'))
        ) {
            this.disableSubmenu();
        }
    };
    ...
    private handlePopupReposition = () => {
        const submenuSlot: HTMLSlotElement | null = this.host.renderRoot.querySelector("slot[name='submenu']");
        const menu = submenuSlot?.assignedElements({ flatten: true }).filter(el => el.localName === 'ts-menu')[0];
    ...
    renderSubmenu() {
        ...
        // Always render the slot, but conditionally render the outer <ts-popup>
        if (!this.isConnected) {
