...
import type { TsQrCode } from './index.js';
import '@tuvsud/design-system/qr-code';

const getCanvas = (qrCode: TsQrCode): HTMLCanvasElement => {
...
const expectCanvasToHaveAriaLabel = (qrCode: TsQrCode, expectedLabel: string): void => {
...
class Color {
    ...
    equals(other: Color): boolean {
        return (
            other === this ||
            (this.r === other.r && this.b === other.b && this.g === other.g && this.alpha === other.alpha)
        );
    }
...
const getQrCodeColors = (qrCode: TsQrCode): QrCodeColors => {
    const canvas = getCanvas(qrCode);
...
const expectQrCodeColorsToBe = (qrCode: TsQrCode, expectedColors: QrCodeColors): void => {
...
describe('<ts-qr-code>', () => {
    it('should render a component', async () => {
        const qrCode = await fixture<TsQrCode>(html` <ts-qr-code value="test data"></ts-qr-code>`);
    ...
    it('uses the label if given', async () => {
        const qrCode = await fixture<TsQrCode>(html` <ts-qr-code value="test data" label="test label"></ts-qr-code>`);

        expectCanvasToHaveAriaLabel(qrCode, 'test label');
    });

    it('sets the correct color for the qr code', async () => {
        const qrCode = await fixture<TsQrCode>(html` <ts-qr-code value="test data" fill="red"></ts-qr-code>`);

        expectQrCodeColorsToBe(qrCode, { foreground: red, background: white });
    });

    it('sets the correct background for the qr code', async () => {
        const qrCode = await fixture<TsQrCode>(
            html` <ts-qr-code value="test data" fill="red" background="blue"></ts-qr-code>`,
        );
    ...
    it('has the expected size', async () => {
        const qrCode = await fixture<TsQrCode>(html` <ts-qr-code value="test data" size="100"></ts-qr-code>`);
