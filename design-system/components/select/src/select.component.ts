...
import styles from './TsSelectStyles.js';

import { TsPopup } from '../../popup/index.js';
import { TsTag } from '../../tag/index.js';
...
import { TsOption } from '../../option/index.js';
 ...
 * @documentation https://create.tuvsud.com/latest/components/select/develop-ecJkcfpG
 * @status stable
 * @since 1.0
 *
 * @dependency ts-icon
 ...
 *
 * @event ts-change - Emitted when the control's value changes.
 ...
 * @event ts-show - Emitted when the select's menu opens.
 * @event ts-after-show - Emitted after the select's menu opens and all animations are complete.
 * @event ts-hide - Emitted when the select's menu closes.
 * @event ts-after-hide - Emitted after the select's menu closes and all animations are complete.
 * @event ts-invalid - Emitted when the form control has been checked for validity and its
 ...
 */
export default class TsSelectComponent extends ComponentElement implements ComponentFormControl {
    ...
    private typeToSelectTimeout: number = 0;
    private closeWatcher: CloseWatcher | null = null;

    @query('.select') popup!: TsPopup;
    ...
    private readonly selectId = `ts-select-${++TsSelectComponent.idCounter}`;
    private readonly helpTextId = `ts-select-help-text-${TsSelectComponent.idCounter}`;
    private readonly errorTextId = `ts-select-error-text-${TsSelectComponent.idCounter}`;
     ...
     * value attribute will be a space-delimited list of values based on the options selected,
     * and the value property will be an array. **For this reason, values must not contain spaces.**
     */
    @state()
    ...
    /**
     * The preferred placement of the select's menu. Note that the actual placement may vary as
     * needed to keep the listbox inside of the viewport.
     */
    ...
    /**
     * A function that customizes the tags to be rendered when multiple=true. The first argument is the option,
     * the second is the current tag's index.  The function should return either a Lit TemplateResult or a
     * string containing trusted HTML of the symbol to render at the specified value.
     */
    @property() getTag: (option: TsOption, index: number) => TemplateResult | string | HTMLElement = option => {
        return html`
            <ts-tag
                part="tag"
                exportparts="
            ...
            "
                ...
                variant="primary"
                removable
                @ts-remove=${(event: TsRemoveEvent) => this.handleTagRemove(event, option)}
            >
                ${option.getTextLabel()}
            </ts-tag>
        `;
    ...
    private handleDocumentKeyDown = (event: KeyboardEvent) => {
        ...
        const isIconButton = target.closest('ts-icon-button') !== null;

        // Ignore presses when the target is an icon button (e.g. the remove button in <ts-tag>)
        ...
        // Handle enter and space. When pressing space, we allow for type to select behaviors so if there's
        // anything in the buffer we _don't_ close it.
        ...
        if (['ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
            ...
            const currentIndex = this.currentOption ? allOptions.indexOf(this.currentOption) : -1;
            ...
            if (!this.open) {
                ...
                // If an option is already selected, stop here because we want that one to remain highlighted when
                // the listbox opens for the first time
            ...
            this.setCurrentOption(allOptions[newIndex] ?? null);
        }
    ...
    private handleComboboxMouseDown(event: MouseEvent) {
        const path = event.composedPath();
        const isIconButton = path.some(el => el instanceof Element && el.tagName.toLowerCase() === 'ts-icon-button');
    ...
    private handleTagRemove(event: TsRemoveEvent, option: TsOption) {
        event.stopPropagation();
    ...
    // Gets an array of all <ts-option> elements
    private getAllOptions() {
        return [...this.querySelectorAll<TsOption>('ts-option')];
    }

    // Gets the first <ts-option> element
    private getFirstOption() {
        return this.querySelector<TsOption>('ts-option');
    }
    ...
    private toggleOptionSelection(option: TsOption, force?: boolean) {
    ...
    protected get tags() {
        return this.selectedOptions.map((option, index) => {
            if (index < this.maxOptionsVisible || this.maxOptionsVisible <= 0) {
                ...
                return html`<div @ts-remove=${(e: TsRemoveEvent) => this.handleTagRemove(e, option)}>
                    ${typeof tag === 'string' ? unsafeHTML(tag) : tag}
                </div>`;
            } else if (index === this.maxOptionsVisible) {
                // Hit tag limit
                return html`<ts-tag size=${this.size} variant="primary"
                    >+${this.selectedOptions.length - index}</ts-tag
                >`;
            }
            return html``;
        });
    ...
    override attributeChangedCallback(name: string, oldVal: string | null, newVal: string | null) {
        ...
        /** This is a backwards compatibility call. In a new major version we should make a clean separation between
         *  "value" the attribute mapping to "defaultValue" property and "value" the property not reflecting.
         */
        if (name === 'value') {
            const cachedValueHasChanged = this.valueHasChanged;
    ...
    override render() {
        ...
        if (!showErrorText && hasHelpText) describedByIds.push(this.helpTextId);
                    ...
                    >
                        ...
                        >
                                ...
                                ? html`
                                      ...
                                      >
                                          <slot name="clear-icon">
                                              <ts-icon name="x-circle-fill" library="system"></ts-icon>
                                          </slot>
                                      </button>
                                  `
                                : ''}
                            ...
                            <slot name="expand-icon" part="expand-icon" class="select__expand-icon">
                                <ts-icon library="system" name="chevron-down"></ts-icon>
                            </slot>
                        </div>
