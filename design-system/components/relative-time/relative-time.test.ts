...
import type { TsRelativeTime } from './index.js';
...
const extractTimeElement = (relativeTime: TsRelativeTime): HTMLTimeElement | null => {
    return relativeTime.shadowRoot?.querySelector('time') || null;
};

const expectFormattedRelativeTimeToBe = async (relativeTime: TsRelativeTime, expectedOutput: string): Promise<void> => {
...
const createRelativeTimeWithDate = async (relativeDate: Date): Promise<TsRelativeTime> => {
    const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
        <ts-relative-time lang="en-US"></ts-relative-time>
    `);
...
describe('ts-relative-time', () => {
    ...
    describe('handles time correctly', () => {
        ...
        testCases.forEach(testCase => {
            ...
            it(`shows the correct relative time given a String object: ${testCase.expectedOutput}`, async () => {
                ...
                const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
                    <ts-relative-time lang="en-US" date="${dateString}"></ts-relative-time>
                `);
        ...
        it('always shows numeric if requested via numeric property', async () => {
            const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
                <ts-relative-time lang="en-US" numeric="always"></ts-relative-time>
            `);
        ...
        it('shows human readable form if appropriate and numeric property is auto', async () => {
            const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
                <ts-relative-time lang="en-US" numeric="auto"></ts-relative-time>
            `);
            relativeTime.date = yesterday;
        ...
        it('allows to use a short form of the unit', async () => {
            ...
            const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
                <ts-relative-time lang="en-US" numeric="always" format="short"></ts-relative-time>
            `);
        ...
        it('allows to use a long form of the unit', async () => {
            const twoYearsAgo = new Date(currentTime.getTime() - 2 * nonLeapYearInSeconds);
            const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
                <ts-relative-time lang="en-US" numeric="always" format="long"></ts-relative-time>
            `);
            relativeTime.date = twoYearsAgo;
        ...
        it('is formatted according to the requested locale', async () => {
            const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
                <ts-relative-time lang="de-DE" numeric="auto"></ts-relative-time>
            `);
            relativeTime.date = yesterday;
    ...
    it('does not display a time element on invalid time string', async () => {
        ...
        const relativeTime: TsRelativeTime = await fixture<TsRelativeTime>(html`
            <ts-relative-time lang="en-US" date="${invalidDateString}"></ts-relative-time>
        `);
