...
import { TsTooltip } from '../../tooltip/index.js';
import styles from './TsCopyButtonStyle.js';
import type { CSSResultGroup } from 'lit';

/**
 * @summary Copies text data to the clipboard when the user clicks the trigger.
 * @documentation https://create.tuvsud.com/latest/components/copy-button/develop-XNVeePDW
 * @status stable
 * @since 1.0
 *
 * @dependency ts-icon
 ...
 *
 * @event ts-copy - Emitted when the data has been copied.
 * @event ts-error - Emitted when the data could not be copied.
 *
 * @slot copy-icon - The icon to show in the default copy state. Works best with `<ts-icon>`.
 * @slot success-icon - The icon to show when the content is copied. Works best with `<ts-icon>`.
 * @slot error-icon - The icon to show when a copy error occurs. Works best with `<ts-icon>`.
 *
 ...
 */
export default class TsCopyButtonComponent extends ComponentElement {
    static override styles: CSSResultGroup = [componentStyles, styles];
    static override dependencies = {
        'ts-icon': TsIcon,
        'ts-tooltip': TsTooltip,
    };
    ...
    @query('ts-tooltip') tooltip!: TsTooltip;
     ...
     * are present, this value will take precedence. By default, the target element's `textContent` will be copied.
            ...
            >
                ...
                >
                    <slot part="copy-icon" name="copy-icon">
                        <ts-icon library="system" name="copy"></ts-icon>
                    </slot>
                    <slot part="success-icon" name="success-icon" hidden>
                        <ts-icon library="system" name="check"></ts-icon>
                    </slot>
                    <slot part="error-icon" name="error-icon" hidden>
                        <ts-icon library="system" name="x-lg"></ts-icon>
                    </slot>
                </button>
            </ts-tooltip>
        `;
