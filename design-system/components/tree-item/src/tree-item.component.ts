...
import { TsCheckbox } from '../../checkbox/index.js';
import { TsSpinner } from '../../spinner/index.js';
import styles from './TsTreeItemStyles.js';
 ...
 * @documentation https://create.tuvsud.com/latest/components/tree-item/develop-jTGo09EH
 * @status stable
 * @since 1.0
 *
 * @dependency ts-checkbox
 * @dependency ts-icon
 * @dependency ts-spinner
 *
 * @event ts-expand - Emitted when the tree item expands.
 * @event ts-after-expand - Emitted after the tree item expands and all animations are complete.
 * @event ts-collapse - Emitted when the tree item collapses.
 * @event ts-after-collapse - Emitted after the tree item collapses and all animations are complete.
 * @event ts-lazy-change - Emitted when the tree item's lazy state changes.
 * @event ts-lazy-load - Emitted when a lazy item is selected. Use this event to asynchronously load data and append
 ...
 */
export default class TsTreeItemComponent extends ComponentElement {
    ...
    private async animateCollapse() {
        ...
        this.emit('ts-after-collapse');
    }

    // Checks whether the item is nested into an item
    private isNestedItem(): boolean {
        const parent = this.parentElement;
        return !!parent && TsTreeItemComponent.isTreeItem(parent);
    }
    ...
    protected override willUpdate(
        changedProperties: PropertyValueMap<TsTreeItemComponent> | Map<PropertyKey, unknown>,
    ) {
    ...
    handleLazyChange() {
        this.emit('ts-lazy-change');
    }

    /** Gets all the nested tree items in this node. */
    getChildrenItems({ includeDisabled = true }: { includeDisabled?: boolean } = {}): TsTreeItemComponent[] {
        return this.childrenSlot
            ? [...this.childrenSlot.assignedElements({ flatten: true })].filter(
                  (item): item is TsTreeItemComponent =>
                      TsTreeItemComponent.isTreeItem(item) &&
                      (includeDisabled || !(item as TsTreeItemComponent).disabled),
              )
            : [];
    }
          ...
          "
                >
                    ...
                    >
                        ${when(
                            this.loading,
                            () => html` <ts-spinner part="spinner" exportparts="base:spinner__base"></ts-spinner> `,
                        )}
                        <slot class="tree-item__expand-icon-slot" name="expand-icon">
                            <ts-icon library="system" name=${isRtl ? 'chevron-left' : 'chevron-right'}></ts-icon>
                        </slot>
