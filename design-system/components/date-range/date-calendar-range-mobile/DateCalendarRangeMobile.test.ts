...
describe('<ts-date-calendar-range-mobile>', () => {
    it('renders one month', async () => {
        const el = await fixture(html`<ts-date-calendar-range-mobile></ts-date-calendar-range>`);
        await (el as LitElement).updateComplete;
        const root = el.shadowRoot;
        const labels = root!.querySelectorAll('.month-label');
        await expect(labels.length).to.equal(1);
    });
    ...
    it('renders weekday headers', async () => {
        ...
        await expect(dow.length).to.equal(7);
    });
    ...
    it('selects start date and emits ts-date-range-select', async () => {
        const el = await fixture(html`<ts-date-calendar-range-mobile></ts-date-calendar-range>`);
        await (el as LitElement).updateComplete;
        const root = el.shadowRoot;
        ...
        const ev = await wait;
        await expect(ev.detail.start instanceof Date).to.equal(true);
        await expect(ev.detail.end).to.equal(undefined);
    });

    it('selects end date and emits complete range', async () => {
        const el = await fixture(html`<ts-date-calendar-range-mobile></ts-date-calendar-range>`);
        await (el as LitElement).updateComplete;
        const root = el.shadowRoot;
        const days = Array.from(root!.querySelectorAll('button')).filter(b => !b.classList.contains('invisible-day'));
        ...
        const ev2 = await w2;
        await expect(ev2.detail.start instanceof Date).to.equal(true);
        await expect(ev2.detail.end instanceof Date).to.equal(true);
    });
