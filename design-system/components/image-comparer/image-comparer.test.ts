...
import type { TsImageComparer } from './index.js';
import '@tuvsud/design-system/image-comparer';

describe('<ts-image-comparer>', () => {
    it('should render a basic before/after', async () => {
        const el = await fixture<TsImageComparer>(html`
            <ts-image-comparer>
                <div slot="before"></div>
                <div slot="after"></div>
            </ts-image-comparer>
    ...
    it('should emit change event when position changed manually', async () => {
        ...
        el.addEventListener('ts-change', handler, { once: true });
