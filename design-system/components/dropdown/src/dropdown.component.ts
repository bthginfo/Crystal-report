...
import type { TsButton } from '../../button/index.js';
import type { TsIconButton } from '../../icon-button/index.js';
import type { TsMenu } from '../../menu/index.js';
...
export default class TsDropdownComponent extends ComponentElement {
    ...
    getMenu() {
        return this.panel.assignedElements({ flatten: true }).find(el => el.tagName.toLowerCase() === 'ts-menu') as
            | TsMenu
            | undefined;
    }
    ...
    private handleDocumentKeyDown = (event: KeyboardEvent) => {
        ...
        if (event.key === 'Tab') {
            ...
            if (this.open && document.activeElement?.tagName.toLowerCase() === 'ts-menu-item') {
    ...
    private handlePanelSelect = (event: TsSelectEvent) => {
        ...
        if (!this.stayOpenOnSelect && target.tagName.toLowerCase() === 'ts-menu') {
            this.hide();
            this.focusOnTrigger();
        }
    };
