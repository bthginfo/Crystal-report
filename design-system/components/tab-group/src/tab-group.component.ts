...
import { TsResizeObserver } from '../../resize-observer/index.js';
import styles from './TsTabGroupStyles.js';
import type { CSSResultGroup } from 'lit';
import type { TsTab } from '../../tab/index.js';
import type { TsTabPanel } from '../../tab-panel/index.js';
...
export default class TsTabGroupComponent extends ComponentElement {
    ...
    private getAllPanels() {
        return [...this.body.assignedElements()].filter(el => el.tagName.toLowerCase() === 'ts-tab-panel') as [
            TsTabPanel,
        ];
    }
    ...
    private handleKeyDown(event: KeyboardEvent) {
        ...
        if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
            ...
            if (activeEl?.tagName.toLowerCase() === 'ts-tab') {
                if (event.key === 'Home') {
    ...
    updateScrollControls() {
        if (this.noScrollControls) {
            ...
            // In most cases, we can compare scrollWidth to clientWidth to determine if scroll controls should show.
            // However, Safari appears to calculate this incorrectly when zoomed at 110%, causing the controls to toggle
            // indefinitely. Adding a single pixel to the comparison seems to resolve it.
            //
