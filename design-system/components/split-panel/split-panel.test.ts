...
import type { TsSplitPanel } from './index.js';
import '@tuvsud/design-system/split-panel';
import { dragElement } from '../../utils/internal/test.js';

const DIVIDER_WIDTH_IN_PX = 4;

const getPanel = (splitPanel: TsSplitPanel, testid: string): HTMLElement => {
    const startPanel = queryByTestId<HTMLElement>(splitPanel, testid);
    expect(startPanel).not.to.be.null;
    return startPanel!;
};

const getPanelWidth = (splitPanel: TsSplitPanel, testid: string) => {
    const panel = getPanel(splitPanel, testid);
    const { width } = panel.getBoundingClientRect();
    return width;
};

const getPanelHeight = (splitPanel: TsSplitPanel, testid: string) => {
    const panel = getPanel(splitPanel, testid);
    const { height } = panel.getBoundingClientRect();
    return height;
};

const getDivider = (splitPanel: TsSplitPanel): Element => {
    const divider = splitPanel.shadowRoot?.querySelector('[part="divider"]');
    expect(divider).not.to.be.null;
    return divider!;
};

describe('<ts-split-panel>', () => {
    afterEach(async () => {
        await resetMouse();
    });

    it('should render a component', async () => {
        const splitPanel = await fixture(html` <ts-split-panel></ts-split-panel> `);
    ...
    describe('panel sizing horizontal', () => {
        it('has two evenly sized panels by default', async () => {
            const splitPanel = await fixture<TsSplitPanel>(
                html`<ts-split-panel>
        ...
        it('changes the sizing of the panels based on the position attribute', async () => {
            const splitPanel = await fixture<TsSplitPanel>(
                html`<ts-split-panel position="25">
                    <div slot="start" data-testid="start-panel">Start</div>
                    <div slot="end" data-testid="end-panel">End</div>
                </ts-split-panel>`,
            );
        ...
        it('emits the ts-reposition	event on position change', async () => {
            const splitPanel = await fixture<TsSplitPanel>(
                html`<ts-split-panel>
                    <div slot="start">Start</div>
                    <div slot="end">End</div>
            ...
            const repositionPromise = oneEvent(splitPanel, 'ts-reposition');
            splitPanel.position = 10;
    ...
    describe('panel sizing vertical', () => {
        it('has two evenly sized panels by default', async () => {
            const splitPanel = await fixture<TsSplitPanel>(
                html`<ts-split-panel vertical style="height: 400px;">
                    <div slot="start" data-testid="start-panel">Start</div>
                    <div slot="end" data-testid="end-panel">End</div>
                </ts-split-panel>`,
            );
        ...
        it('changes the sizing of the panels based on the position attribute', async () => {
            const splitPanel = await fixture<TsSplitPanel>(
                html`<ts-split-panel position="25" vertical style="height: 400px;">
                    <div slot="start" data-testid="start-panel">Start</div>
                    <div slot="end" data-testid="end-panel">End</div>
                </ts-split-panel>`,
            );
        ...
        it('cannot be resized if disabled', async () => {
            const splitPanel = await fixture<TsSplitPanel>(
                html`<ts-split-panel disabled vertical style="height: 400px;">
                    <div slot="start">Start</div>
                    <div slot="end">End</div>
                </ts-split-panel>`,
            );
