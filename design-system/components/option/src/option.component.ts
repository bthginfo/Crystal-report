...
import styles from './TsOptionStyles.js';
import type { CSSResultGroup } from 'lit';
import type { TsSelect } from '../../select/index.js';

/**
 * @summary Options define the selectable items within various form controls such as [select](/components/select).
 * @documentation https://create.tuvsud.com/latest/components/option/develop-8AWP4IEt
 * @status stable
 * @since 1.0
 *
 * @dependency ts-icon
 ...
 */
export default class TsOptionComponent extends ComponentElement {
    ...
    private handleDefaultSlotChange() {
        if (this.isInitialized) {
            ...
            customElements.whenDefined('ts-select').then(() => {
                const controller: TsSelect = this.closest('ts-select')!;
                if (controller) {
